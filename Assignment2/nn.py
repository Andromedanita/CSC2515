...
from util import LoadData, Load, Save, DisplayPlot, ShowWeights
import sys
import numpy as np
...
def InitNN(num_inputs, num_hiddens, num_outputs):
    ...
    print
    print ("W1 shape is:", np.shape(W1))
    print ("W2 shape is:", np.shape(W2))
    print ("W3 shape is:", np.shape(W3))
    
    print ("b1 shape is:", np.shape(b1))
    print ("b2 shape is:", np.shape(b2))
    print ("b3 shape is:", np.shape(b3))
    print
...
def Train(model, forward, backward, update, eps, momentum, num_epochs,
    ...
    #print ("NUMBER of CASES:",num_train_cases)
    if batch_size == -1:
    ...
    plt.ion()
    folder = "/Users/anita/Documents/Grad_Second_Year/CSC2515/assignment2/Results/Q3/Q3.3/"
    ...
    plt.subplot(121)
    plt.plot(np.array(train_ce)[:, 0], np.array(train_ce)[:, 1], 'b')
    plt.plot(np.array(valid_ce)[:, 0], np.array(valid_ce)[:, 1], 'g')
    ...
    plt.savefig(folder+"Q3.4_cnn_Q3.2_eps{0}_epoch{1}_batch{2}_hidden.png".format(eps, num_epochs,batch_size))
...
def main():
    ...
    num_hiddens = [7,13] # default [16, 32]
    eps         = 0.01
    momentum    = 0.0
    num_epochs  = 1000
    batch_size  = 100
    ...
    model_fname = 'nn_model_eps_{0}_mom_{1}_epoch_{2}_batch_{3}.npz'.format(eps, momentum, num_epochs,batch_size)
    stats_fname = 'nn_stats_eps_{0}_mom_{1}_epoch_{2}_batch_{3}.npz'.format(eps, momentum, num_epochs,batch_size)
